<?php
            ...
            foreach($data->find('div.highlight') as $highlight){
                ...
                foreach($highlight_text as $text) {
                    ...
                    if (preg_match('/Отправление получено/', $text)) {
                        ...
                        if (isset($results['backshipment']) && $results['backshipment'] ==
    ...
    $query_params = array(':order_id' => isset($_GET['id']) ? $_GET['id'] : null);
    ...
    while($order = $stmt->fetch()) { //echo $order['newpost_id'];
        ...
        if(!empty($matches)) {
            ...
            if($status_array['cargo_arrive_backshipment']['status_flag']) {
                ...
                $matches['cash_amount'] = isset($results['cash_amount']) ?: null;
        ...
        if ($order['status_step2'] == '0' or in_array($order['status_step2'], isset($matches['from_status2']) ? $matches['from_status2'] : array())) {
            $match_result['new_s2'] = $match_result['status_step2'];
        }
        else {
            $match_result['new_s2'] = $order['status_step2'];
        }
        if ((isset($matches['status_step3']) or (isset($matches['status_step3']) and $matches['status_step3']=='0')) and ($order['status_step3'] == '0' or in_array($order['status_step3'], $matches['from_status3'] ? $matches['from_status3'] : array()))) {
            $match_result['new_s3'] = $matches['status_step3'];
        } else {
            $match_result['new_s3'] = $order['status_step3'];
        }

        $update_query = "UPDATE orders SET ".(($order['status_step2'] != $match_result['new_s2']) ? ('status_step2 = :new_s2, '.(isset($match_result['alert_at']) ? "alert_at=".$match_result['alert_at'].", ":"")) : '').($order['status_step3'] != $match_result['new_s3'] ? 'status_step3 = :new_s3, ' : '')." newpost_answer=:answer, newpost_last_update=NOW() ".(isset($match_result['id_back']) ? ", newpost_backorder=:id_back" : "")." WHERE id = :order_id";
        if (isset($match_result['id_back'])) {
        ...
        catch(PDOException $ex){ echo ("Невозможно выполнить запрос5: ".$update_query."<br>".print_r($update_query_params,true)."<br>". $ex->getMessage()); }

        if ($order['status_step2'] != $match_result['new_s2'] || $order['status_step3'] != $match_result['new_s3']) {
         $aquery = "INSERT INTO orders_audit(date, order_id, user_id, activity, comment) VALUES
                     (   NOW(),
                         :order_id,
                         :user_id,
                         :activity,
         ...
         $aquery_params = array(
             ':comment' => $match_result['msg'] ? $match_result['msg'] : '',
...
//print_r($matches);
    ...
    try{
        ...
        die("Невозможно выполнить запрос3: " . $ex->getMessage());
    }
    ...
    while($order = $stmt->fetch()) { //echo $order['newpost_backorder'];
     ...
     $update_query = "UPDATE orders SET ".((isset($match_result['status_step3']) and $match_result['status_step3'] != '0' and $order['status_step3'] != $match_result['status_step3']) ? 'status_step3 = :status_step3, ' : '')." newpost_backorder_answer=:answer, newpost_last_backorder_update=NOW() WHERE id = :order_id";
     $update_query_params = array(':order_id' => $order['id'], ':answer' => json_encode($match_result));

     if (isset($match_result['status_step3']) and $match_result['status_step3'] != '0' and $order['status_step3'] != $match_result['status_step3']) {
         $update_query_params = array_merge($update_query_params, array(':status_step3' => $match_result['status_step3']));
     }

     try{    $update_stmt = $db->prepare($update_query);
             $update_result = $update_stmt->execute($update_query_params); }
     catch(PDOException $ex){ echo ("Невозможно выполнить запрос6: " . $ex->getMessage()); }
